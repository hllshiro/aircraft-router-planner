...
### Fixed
- fix(altitude): 三段式垂直剖面巡航段归零——将 `apply_climb_rate` 合并到 `apply_vertical_profile` 内部，避免 FMM 网格离散化的近距离点对产生假爬升率连锁压低所有中间点
...
### Changed
- 优化地形参数帮助文本：简化索引 id 重复显示，统一为"可用数据"标签

### Fixed
- fix(radar): 修正雷达有效半径计算公式，cap 100km 限制膨胀偏差而非最终半径
