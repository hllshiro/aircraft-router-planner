...
fn smooth_empty_path_no_panic() {
    ...
    assert_eq!(theta_star_smooth(&p, &check, None, None, 0.0).len(), 0);
...
fn smooth_single_point_no_panic() {
    ...
    let check = always_true();
    assert_eq!(theta_star_smooth(&p, &check, None, None, 0.0).len(), 1);
...
fn smooth_nan_inf_no_panic() {
    ...
    let check = always_true();
    let _ = theta_star_smooth(&bad, &check, None, None, 0.0);
...
fn smooth_extreme_geometry_no_panic() {
    ...
    let check = always_true();
    let _ = theta_star_smooth(&p, &check, None, None, 0.0);
...
fn smooth_chain_degenerate_no_panic() {
    ...
    let check = always_true();
    let chain: Vec<Box<dyn Smoother>> = vec![Box::new(ThetaStarSmoother { check: &check, max_turn_deg: None, entry_heading: None, min_r_m: 0.0 })];
