...
/// 圆心（R 归一化，最终乘 R）。
fn center(p: (f64, f64), th: f64, t: Turn) -> (f64, f64) {
    match t {
        Turn::L => (p.0 - th.sin(), p.1 + th.cos()),
        Turn::R => (p.0 + th.sin(), p.1 - th.cos()),
    }
}
...
pub fn dubins_path(
    ...
    for t1 in [Turn::L, Turn::R] {
        for t2 in [Turn::L, Turn::R] {
            let c1 = center(p0, th0, t1);
            let c2 = center(p1, th1, t2);
            let vx = c2.0 - c1.0;
            let vy = c2.1 - c1.1;
