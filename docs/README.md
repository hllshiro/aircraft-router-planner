...
> 本文档集将"设计蓝图"与"代码实现"映射起来，标注已实现项、占位项与未实现项（截至 2026-08-14）。
...
- **2026-08-14 要点**：起终点贴地（低于地面 MSL 才抬升，不因中间地形抬升）+ 中间段 100m 净空保底 + 方案 B 自动中间锚点 + terrain_follow_insert（250m 峰顶捕获、起终点段 15° 爬升/下降线插点）+ apply_climb_rate（双向 ≤15°）——必经点穿山三轮修复（docs/07 §5e/§6.6、docs/08 §7.5）；Demo 底图三源 + 视口瓦片系统 + 镜像修复（z=-北 右手系）+ 正南高空俯视（docs/09）；zigzag26/37/38 回归全量 261+33+2+2+1 过（docs/10）
